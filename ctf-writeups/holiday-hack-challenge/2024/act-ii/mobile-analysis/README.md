...
> **Note:** The actual path when using Jadx GUI depends on the file loaded:
...
| File | Description |
|---|---|
| `SantaSwipe.apk` | Debug APK — analyzed for Silver |
| `SantaSwipeSecure.aab` | Release AAB — analyzed for Gold |
| `universal.apk` | Universal APK extracted from the AAB via `bundletool` |
| `decrypt_data.py` | Python script to decrypt the AES-GCM encrypted SQL query and name |
