...
Upon clicking the "Download" button, we get link to three files:
...
The files starting with `log_chunk_` are zipped log files, and the `elf-stack-siem-with-logs.zip` file contains the Elf Stack SIEM setup.
...
| File | Description |
|---|---|
| `files/elf-stack-siem-with-logs.zip` | Full ELK stack with pre-loaded logs for local analysis |
| `files/log_chunk_1.log.zip` | First chunk of raw log data |
| `files/log_chunk_2.log.zip` | Second chunk of raw log data |
