...
- [Santa Vision](#santa-vision)
  ...
  - [Santa Vision A](#santa-vision-a-1)
    - [Silver](#silver)
      - [Analysis](#analysis)
      - [Solution](#solution)
    - [Gold](#gold)
      - [Hint 5: Like a Good Header on Your HTTP? (B)](#hint-5-like-a-good-header-on-your-http-b)
      - [Gold Hint 1](#gold-hint-1)
      - [Analysis](#analysis-1)
      - [Solution](#solution-1)
  - [Santa Vision B](#santa-vision-b-1)
    - [Silver](#silver-1)
      - [Analysis](#analysis-2)
      - [Solution](#solution-2)
    - [Gold](#gold-1)
      - [Hint 6: Looking Deeper (C)](#hint-6-looking-deeper-c)
      - [Gold Hint 2](#gold-hint-2)
      - [Analysis](#analysis-3)
      - [Solution](#solution-3)
  - [Santa Vision C](#santa-vision-c-1)
    - [Silver](#silver-2)
      - [Analysis](#analysis-4)
      ...
      - [Solution](#solution-4)
    - [Gold](#gold-2)
      - [Gold Hint 3](#gold-hint-3)
      - [Analysis](#analysis-5)
      - [Solution](#solution-5)
  - [Santa Vision D](#santa-vision-d-1)
    - [Silver](#silver-3)
      - [Analysis](#analysis-6)
      - [Solution](#solution-6)
    - [Gold](#gold-3)
      - [Gold Hint 4](#gold-hint-4)
      - [Analysis](#analysis-7)
      - [Solution](#solution-7)
...
1. Port 1883 (MQTT):
   - MQTT is a message broker protocol. Given the challenge instructions, this is critical for accessing topics like `northpolefeeds` and `frostbitfeed` and managing admin privileges.
   - We can use the Mosquitto CLI or MQTTX to connect and explore topics.
     ```bash
     mosquitto_sub -h 35.223.215.14 -p 1883 -t '#' -v
     ```
   - If authentication is required, we may need to find credentials in another service or via enumeration.

2. Port 8000 (HTTP - Gunicorn):
   - This is a Python WSGI HTTP server. Likely hosting the "portal" mentioned in the challenge.
   - We can navigate to the web application in a browser:
     ```bash
     http://35.223.215.14:8000
     ```
   - We can also use tools like `gobuster` or `dirsearch` to enumerate directories and locate sensitive files:
     ```
     gobuster dir -u http://35.223.215.14:8000 -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt
     ```
...
3. Port 9001 (TOR? or Management Interface):
   ...
   - We can attempt connecting via a browser or using `telnet`:
     ```bash
     telnet 35.223.215.14 9001
     ```
   - If it’s an HTTP-based service, we can test with a browser:
     ```bash
     http://35.223.215.14:9001
     ```

Let's check the Python web server first. Upon loading the page at URL http://35.223.215.14:8000 we can see the **Santa Vision** portal asking for a username and password.

The page at the bottom has a label `(topic ‘sitestatus’ available.)`. Maybe this hints at an MQTT topic.

Looking at the HTML, we can see a comment:
```html
...
* For the camera feed server we will use the same portal IP address, However, for the port, we will use the last port from the nmap scan that was unrecognized:
  9001/tcp open     tor-orport?
...
```
The images can be seen under the [northpolefeeds/](./northpolefeeds/) folder. The messages from the images are:
...
* The letters seem to the scrambled or rotated. After testing several ROT shift values, it turns out that the name is encoded with a ROT-10. After decoding the word, the Gold word is `Snowmobile`.
...
The default images in the `northpolefeeds` feed (available under the [northpolefeeds/](./northpolefeeds/) folder) do not show Santa.
...
The images are available under the [northpolefeeds/silver/](./northpolefeeds/silver/) folder. They show Santa on a Pogo Stick
...
The images are available under the [northpolefeeds/gold/](./northpolefeeds/gold/) folder. They show Santa riding a different contraption: a Hover Craft.
...
| File | Description |
|---|---|
| `applicationDefault.bin` | JFFS2 filesystem image analyzed for credentials |
| `SantasTopSecretDB-2024-Z.sqlite` | SQLite database extracted from the challenge infrastructure |
