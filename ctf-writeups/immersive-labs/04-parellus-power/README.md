...
You get a password-protected ZIP file that contains a couple of plain text files with example invoices (`Eldercherry Parellus Invoice.txt`, `Herocoms Parellus Invoice.txt`), a couple of encrypted files (`Arcadia Blossom Parellus Invoice.txt.encoded`, `Parellus Power Finances - Jan 18 - Jan 19.csv.encoded`) and a Python script (`encrypt.py`) that shows how the encryption is done.
...
Testing with the encrypt and decrypt scripts with a key of your choice can confirm the decryption is working as expected.
...
| `findings.txt` | Challenge notes and key recovery walkthrough |
...
- [`ctf-techniques/crypto/`](../../../ctf-techniques/crypto/) — cryptography technique reference
