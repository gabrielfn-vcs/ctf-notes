# Lab 1 - Phishing for Treats - Halloween Edition

## Table of Contents
- [Lab 1 - Phishing for Treats - Halloween Edition](#lab-1---phishing-for-treats---halloween-edition)
  - [Table of Contents](#table-of-contents)
  - [Overview](#overview)
  - [Analysis](#analysis)
  - [Solution](#solution)

## Overview

Need to check messages in the provided web app to find the answer to the question.

## Analysis

There is one message from Count Dracula that provides the required information.
![`dracula.png`](./dracula.png)

## Solution

Who did Count Dracula intend to send the love letter to?

You should make a note of the answer to this question, as you'll need it to escape through the Emergency Exit.

**Answer:** `Morticia`
