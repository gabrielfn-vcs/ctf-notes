...
> **Tip:** Flags and tokens are sometimes hidden as subdomain names in CNAME records (e.g., `the.token.is.FLAG.domain.com`), not just in TXT record values. Scan all record names, not just their values.
...
- [HackTricks - DNS Zone Transfer](https://book.hacktricks.xyz/network-services-pentesting/pentesting-dns#zone-transfer)
