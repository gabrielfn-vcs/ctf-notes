...
Let's take advantage of the `/opt/security-sweep` file:

- Can be edited by anyone.
- It is owned by `root`.
- It executes frequently in `/etc/crontab`.

and the `/opt/complaints-desk/backlog` directory:

- It allows anyone to read or write.

Let's modify the file to insert the `cat /root/token` command and dump it to a file under `/opt/complaints-desk/backlog`, which is open to write anything.

1. Create a new version of the file on the client side.
   ```bash
   $cat security-sweep
...
| | |
|:---|---:|
| ← [Confusing Code](../lab-5-confusing-code/README.md) | [Fearsome Forensics](../lab-7-fearsome-forensics/README.md) → |
