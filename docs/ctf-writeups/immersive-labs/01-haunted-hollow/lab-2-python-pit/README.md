...
- [Lab 2 - Python Pit](#lab-2---python-pit)
  ...
  - [Setup](#setup)
    ...
    - [Activity During Application Start](#activity-during-application-start)
      - [Login Request](#login-request)
      - [Signup Request](#signup-request)
    - [Login Request from Form](#login-request-from-form)
  - [Analysis](#analysis)
...
Let's update the `/auth/login` endpoint in the [`auth.py`](./THEIA/auth.py) file to provide INFO-level log details for the following:
...
1. The code in [`dashboard.py`](./THEIA/dashboard.py) contains the logic to check for the rate values, but they are inconsistent:
   ...
   - Need to update the code to have a consistent range check for the input `rate` across all the endpoints.
     ```python
...
2. The [`signup.html`](./THEIA/templates/signup.html) file includes a hidden field for `admin` defaulted to 0. However, in the [`auth.py`](./THEIA/auth.py) file, there is no check to confirm that the value can be manipulated to 1 with a man-in-the-middle attack.
   - Need to change the logic in the `/auth/signup` endpoint in the [`auth.py`](./THEIA/auth.py) file to set `admin = 0` and ignore the `request.form["admin"]` value.

3. The [`login.html`](./THEIA/templates/login.html) file has a comment with credentials to a `PyPit` account.
   ```html
...
4. The [`dashboard.html`](./THEIA/templates/dashboard.html) file has a comment with the code needed for later.
   ```html
...
| | |
|:---|---:|
| ← [The Gatekeepers](../lab-1-the-gatekeepers/README.md) | [The Cursed Crypt](../lab-3-the-cursed-crypt/README.md) → |
