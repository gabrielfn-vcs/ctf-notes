...
- [On the Wire](#on-the-wire)
  ...
  - [Analysis](#analysis)
    ...
    - [1-Wire Protocol and Message Format](#1-wire-protocol-and-message-format)
      - [1-Wire Background](#1-wire-background)
      - [1-Wire Messages (`dq` wire)](#1-wire-messages-dq-wire)
    - [SPI Protocol and Message Format](#spi-protocol-and-message-format)
      - [SPI Background](#spi-background)
      - [SPI Messages (`mosi` wire)](#spi-messages-mosi-wire)
      - [SPI Messages (`sck` wire)](#spi-messages-sck-wire)
    - [I2C Protocol and Message Format](#i2c-protocol-and-message-format)
      - [I2C Background](#i2c-background)
      - [I2C Messages (`sda` wire)](#i2c-messages-sda-wire)
      - [I2C Messages (`scl` wire)](#i2c-messages-scl-wire)
    - [High-Level Strategy](#high-level-strategy)
  - [Solution](#solution)
    - [Stage 1: 1-Wire Data](#stage-1-1-wire-data)
      - [1-Wire Data Capture](#1-wire-data-capture)
      - [1-Wire Data Analyze](#1-wire-data-analyze)
      - [1-Wire Data Decode](#1-wire-data-decode)
      - [1-Wire Data Answer](#1-wire-data-answer)
    - [Stage 2: SPI Data](#stage-2-spi-data)
      - [SPI Data Capture](#spi-data-capture)
      - [SPI Data Analyze](#spi-data-analyze)
      - [SPI Data Decode](#spi-data-decode)
      - [SPI Data Answer](#spi-data-answer)
    - [Stage 3: I2C Data](#stage-3-i2c-data)
      - [I2C Data Capture](#i2c-data-capture)
      - [I2C Data Analyze](#i2c-data-analyze)
      - [I2C Data Decode](#i2c-data-decode)
      - [I2C Data Answer](#i2c-data-answer)
  - [Outro](#outro)
  - [Files](#files)
...
Help Evan next to city hall hack this gnome and retrieve the temperature value reported by the I2C device at address `0x3C`. The temperature data is XOR-encrypted, so you'll need to work through each communication stage to uncover the necessary keys. Start with the unencrypted data being transmitted over the 1-wire protocol.
...
3. Identify protocol from wire names (e.g., dq → 1-Wire; mosi/sck → SPI; sda/scl → I2C).
...
### WebSocket Endpoints
In the Network tab of the DevTools, we can confirm all the WebSocket endpoints:
...
### I2C Protocol and Message Format
...
Like SPI, you sample SDA on the rising edge of SCL, and bits are MSB-first. However, I2C has additional complexity:
...
### High-Level Strategy
The goal is to get the reading from the I2C device at address `0x3C`. Using the provided WebSocket endpoints, this is the high-level strategy:

1. Capture and analyze the 1-Wire signal to find information that helps read the SPI signal.
2. Capture and analyze the SPI signal to find information that helps read the I2C signal.
3. Capture and analyze the I2C signal to decrypt the final information and solve the challenge.
...
- `dq` is the 1-Wire data line.
- `t` is the timestamp (looks like microseconds or clock ticks).
...
- The script detects if there are multiple RESET instances to identify multiple frames.
...
#### SPI Data Answer
The SPI data bus decrypted message indicates that the I2C bus data can be decrypted using the XOR key `bananza`.

### Stage 3: I2C Data

#### I2C Data Capture
Let's capture I2C signal data into [`scl.json`](./i2c/scl.json) and [`sda.json`](./i2c/sda.json) files using the [`i2c_capture.py`](./i2c/i2c_capture.py) Python script.

The script captures I2C traffic by leveraging timestamp resets (`t == 0`) as frame boundaries, buffering a complete transaction for both SDA and SCL lines, and preserving timing relationships for accurate clock-driven decoding.
```log
...
#### I2C Data Answer
The I2C data bus decrypted message indicates that the temperature of the device is `32.84`.
...
| File | Description |
|---|---|
| [`1-wire/1-wire_capture.py`](./1-wire/1-wire_capture.py) | Python script to capture 1-Wire DQ signal data via WebSocket |
| [`1-wire/dq.csv`](./1-wire/dq.csv) | CSV file with captured 1-Wire DQ signal data |
| [`1-wire/1-wire_decoder.py`](./1-wire/1-wire_decoder.py) | Python script to decode 1-Wire frames from the CSV file |
| [`spi/spi_capture.py`](./spi/spi_capture.py) | Python script to capture SPI MOSI/SCK signals via WebSocket |
| [`spi/mosi.json`](./spi/mosi.json) | JSON file with captured SPI MOSI signal data |
| [`spi/sck.json`](./spi/sck.json) | JSON file with captured SPI SCK signal data |
| [`spi/spi_decoder.py`](./spi/spi_decoder.py) | Python script to decode SPI frames from MOSI/SCK JSON files |
| [`i2c/i2c_capture.py`](./i2c/i2c_capture.py) | Python script to capture I2C SDA/SCL signals via WebSocket |
| [`i2c/sda.json`](./i2c/sda.json) | JSON file with captured I2C SDA signal data |
| [`i2c/scl.json`](./i2c/scl.json) | JSON file with captured I2C SCL signal data |
| [`i2c/i2c_decoder.py`](./i2c/i2c_decoder.py) | Python script to decode I2C frames from the SDA JSON file |

---

## References

- [`ctf-techniques/hardware/`](../../../../../ctf-techniques/hardware/README.md) — 1-Wire, SPI, and I2C protocol capture and decoding techniques
