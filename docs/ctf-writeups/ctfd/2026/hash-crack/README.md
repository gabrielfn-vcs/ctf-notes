...
- [Golden Hash Crack](#golden-hash-crack)
  ...
  - [Overview](#overview)
  - [Hints](#hints)
    - [Hint 1: Password Policy](#hint-1-password-policy)
    - [Hint 2: Audit Logs](#hint-2-audit-logs)
  - [Analysis](#analysis)
  - [Solution](#solution)
...
* [`leaked_passwords.txt`](./leaked_passwords.txt)
* [`rockyou.txt.zip`](./rockyou.txt.zip)

---

## Hints

### Hint 1: Password Policy
...
The entry in the [`leaked_passwords.txt`](./leaked_passwords.txt) file with the password to crack is the one for `golden_admin`.
...
Let's create a target file with only the line that matters:
```bash
...
The [`filter.py`](./filter.py) Python script was created to eliminate all words from `rockyou.txt` that do not meet the password policy rules:
...
After applying this filter, 135,421 passwords were saved to a new [`filtered.txt`](./filtered.txt) file.
...
| File | Description |
|---|---|
| [`leaked_passwords.txt`](./leaked_passwords.txt) | Leaked factory user database with password hashes |
| [`rockyou.txt.zip`](./rockyou.txt.zip) | Compressed common password wordlist |
| [`golden.hash`](./golden.hash) | Isolated `golden_admin` hash extracted for cracking |
| [`filter.py`](./filter.py) | Script to filter `rockyou.txt` according to the password policy |
| [`filtered.txt`](./filtered.txt) | Filtered wordlist output (135,421 passwords) |

---

## References

- [`ctf-techniques/crypto/`](../../../../ctf-techniques/crypto/README.md) — hash cracking Quick Reference
