...
- [Decrypt the Stolen Recipe Fragment](#decrypt-the-stolen-recipe-fragment)
  ...
  - [Solution](#solution)
    ...
    - [Step 4: Decrypt the file](#step-4-decrypt-the-file)
    - [Step 5: Extract the flag](#step-5-extract-the-flag)
    - [Answer](#answer)
  - [Files](#files)
...
* [`wonka_wordlist.txt`](./wonka_wordlist.txt)
* [`QC_export.txt.gpg`](./QC_export.txt.gpg)
...
The password is expected to contain three (3) words concatenated from [`wonka_wordlist.txt`](./wonka_wordlist.txt).
...
```
This will create a file [`gpg_hash.txt`](./gpg_hash.txt) with the hash we need to crack.
...
The hash is a long string that starts with `$gpg$` and is followed by several fields separated by `*`. Each field contains specific information about the hash, such as the version, the hash type, the cost parameters, and the actual hash value.
...
The given file can be decrypted with the following command:
```bash
gpg --batch --pinentry-mode loopback --passphrase sweetfizzyriver -d QC_export.txt.gpg
```

For convenience, the [`decrypt.py`](./decrypt.py) and [`decrypt.sh`](./decrypt.sh) utility scripts can also be used to decrypt the file.

### Step 5: Extract the flag

Inside the decrypted file [`QC_export.txt`](./QC_export.txt) will be the ingredient codename: that is the flag.
...
| File | Description |
|---|---|
| [`QC_export.txt.gpg`](./QC_export.txt.gpg) | GPG-encrypted file containing the secret recipe |
| [`QC_export.txt`](./QC_export.txt) | Decrypted plaintext output |
| [`gpg_hash.txt`](./gpg_hash.txt) | Hash extracted from the GPG file via `gpg2john` |
| [`wonka_wordlist.txt`](./wonka_wordlist.txt) | Custom wordlist used as the PRINCE input |
| [`decrypt.py`](./decrypt.py) | Python script to decrypt using the recovered passphrase |
| [`decrypt.sh`](./decrypt.sh) | Bash script for interactive passphrase testing |

---

## References

- [`ctf-techniques/crypto/`](../../../../ctf-techniques/crypto/README.md) — GPG passphrase cracking and hash cracking reference
...
| | |
|:---|---:|
| ← [Candy Wrapper](../candy-wrapper/README.md) | [Hash Crack](../hash-crack/README.md) → |
